...
database = pd.read_csv("D:/DeepMASS2_GUI/data/DeepMassStructureDB-v1.1.csv")
inchikeys_database = list(database['Short InChIKey'].values)
print(len([i for i in inchikeys if i[:14] in inchikeys_database]))


sirius_path = "Example/CASMI/sirius_1"
...
for i in range(1, 11):
    ...
    msfinder_ratio = len(np.where(ranking_result['MSFinder Ranking'] <= i )[0]) / (len(ranking_result) - 12)
...
for i in range(1, 11):
    ...
    msfinder_ratio = len(np.where(ranking_result_1['MSFinder Ranking'] <= i )[0]) / (len(ranking_result_1) - 12)
