...
files = os.listdir('data/CASMI2016_Test')
challenge = pd.read_csv('solutions/casmi_2016_challenge_test.csv')

challenge_ms = np.repeat(None, len(challenge))
for i, f in enumerate(tqdm(files)):
    f_ = 'data/CASMI2016_Test/{}'.format(f)
...
save_as_mgf(list(challenge_ms), 'save/casmi_2016_challenge_test.mgf')

'''
for i, s in enumerate(challenge_ms):
    save_as_msp([challenge_ms[i]], 'save/casmi_2016_test_msp/casmi_2016_challenge_{}.msp'.format(i))
    with open('save/casmi_2016_test_msp/casmi_2016_challenge_{}.msp'.format(i)) as msp:
        lines = msp.readlines()
        lines = [l.replace('_', '') for l in lines]
        lines = [l.replace('ADDUCT', 'PRECURSORTYPE') for l in lines]
    with open('save/casmi_2016_test_msp/casmi_2016_challenge_{}.msp'.format(i), 'w') as msp:
