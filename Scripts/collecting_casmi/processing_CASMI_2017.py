...
files = os.listdir('data/CASMI2017')
challenge = pd.read_csv('solutions/casmi_2017_challenge.csv')

challenge_ms = np.repeat(None, len(challenge))
for i, f in enumerate(tqdm(files)):
    f_ = 'data/CASMI2017/{}'.format(f)
    ...
    ion_mode = challenge.loc[i, 'ION_MODE'].lower()
...
save_as_mgf(list(challenge_ms), 'save/casmi_2017_challenge.mgf')

'''
for i, s in enumerate(challenge_ms):
    save_as_msp([challenge_ms[i]], 'save/casmi_2017_msp/casmi_2017_challenge_{}.msp'.format(i))
    with open('save/casmi_2017_msp/casmi_2017_challenge_{}.msp'.format(i)) as msp:
        lines = msp.readlines()
        lines = [l.replace('_', '') for l in lines]
        lines = [l.replace('ADDUCT', 'PRECURSORTYPE') for l in lines]
    with open('save/casmi_2017_msp/casmi_2017_challenge_{}.msp'.format(i), 'w') as msp:
